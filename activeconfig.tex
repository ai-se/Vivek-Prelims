...
rented to multiple customers (called tenants) and  by the tenants\textquotesingle  users over the internet [7]. In addition, application
...
%Furthermore, providing views for different stakeholders can be achieved by applying staged configuration. 
Tenancy contracts define
the provisioned application functionality as well as quality
of service (QoS) guarantees. 
%Thus, an extended domain feature
...
%problem space [8]. 
Some configuration steps, e.g., performed
...
with software evolution of one commercial storage system
and three popular open-source server software projects. 
%Take
%Hadoop MapReduce as an example, its first release in Apr. 2006
%had only 17 parameters, but the release in Oct. 2013 already had
%173, an increase of more than nine times. In accordance with this
%trend, the configuration space will continue to increase. The situation
%is further worsened by the growing number of machines
%that replicate multiple software instances in data centers, and the
%increasing cost of human resources for managing them.



 
 Configuration options have become ubiquitious with all the software systems and with the advent of cloud computing services, performance modelling has become a very important for the service providers to guarantee SLAs while keeping the process profitable. As a service provider, the customers come to them with different software and ask for a particular performance reqriments. The general tendency of the service providers have been to be conservative and over reserve the resource to provide a particular performance which leads to resource wastage. Though there has been numerous attempts by the system software groups around the world to come up with dynamic resource scaling (for more detials look at \cite{naskos2016cloud}), the administrators would still like to start the system with the optimal configurations. This means before the system starts the service provider needs to run various bench mark runs to enable to choose the right configurations. Various runs of the benchmarks translates into wasted time and resources. The effort required for this task is exponentiated if the configurations have to be optimized for multiple performance measure such as energy, infrastructure cost etc. \cite{neha2016}. Since there is no concrete mathematical model available for such problems, it renders all the numerical methods useless. Performing a exhaustive search of the possible options is not feasible, so researchers over the years have come up with various strategies like stochastic search, evoluationary search etc. 
