...
rented to multiple customers (called tenants) and accessed by the tenants’ users over the internet [7]. In addition, application
resources are shared among tenants\cite{chong2007architecture}.
...
as stated in the literature \cite{reiser2007multi, ruehl2011applying, schroeter2012towards}. Furthermore,
providing views for different stakeholders can be achieved by
applying staged configuration. Tenancy contracts define
the provisioned application functionality as well as quality
of service (QoS) guarantees. Thus, an extended domain feature
model (EFM) with attributes is convenient to express
this variability and a staged configuration as proposed by
Czarnecki et al. is applicable to create those contracts in the
problem space [8]. Some configuration steps, e.g., performed
by tenants, are independent from each other. However, others
are dependent, e.g., a tenant\'s configuration choices depend
on the pre-configuration of the provider. Therefore, a structured configuration process is needed. In addition,
stakeholders’ objectives may change over time, e.g., if a tenant
decides to change the tenancy contract. Thus, the con-
figuration process needs to support reconfiguration of stakeholder
...
the solution space.
 ...
 With the importance of configuration in various domain it is inevitable that configuration errors have become one of the major causes for today's system failures. For example Barroso et al. \cite{barroso2013datacenter} report that configuration errors were the second major cause of the service-level failures at one of the Google's main services. Rabkin et al. \cite{rabkin2013hadoop} report that configuration errors were the dominant cause of Hadoop cluster failures, in terms of both the number of customer cases and the supporting time. Similar statistics have been observed in other types of systems and applications like storage systems \cite{jiang2009understanding, yin2011empirical}, large-scale Internet and Web services\cite{oppenheimer2003internet}, cloud systems\cite{gunawi2014bugs} etc. In recent years, we have witnessed that a number of large Internet and cloud-service providers (eg. Google, Microsoft, Amazon, LinkedIn) experienced misconfiguration-induced outages, affecting millions of their customers \cite{thomasthanks, brodkin2012,miller2012, liang2013 }.  Typically, a large, complex software system exposes hundreds of configuration parameters, and each parameter has its setting constraints (i.e., correctness rules or requirements) \cite{kiciman2004discovering, nadi2014mining}; many configuration constraints are neither intuitively designed nor well documented \cite{rabkin2011static, xu2013not}.  Moreover, many configuration parameters have correlations and dependencies with other parrameters or the system\'s runtime environment, which further increases the complexity. A recent study \cite{yin2011empirical} shows that configuration-related issues account for 27\% of all the customer-support cases in a major storage company in the U.S., and are the most significant  contributor (31\%) among all the high-severity support cases. Welsh\cite{welsh2013} ranked \"an escapse from configuration hell\" as the number one problem among all that he wished system reserachers would work on in 2013. In Google Faculty Summit 2011, Wilkes\cite{wilkes2011} named configuration as the next big challenge for the system research.
 
 Configuration options have become ubiquitious with all the software systems and with the advent of cloud computing services, performance modelling has become a very important for the service providers to gurantee SLAs while keeping the process profitable. As a service provider, the customers come to them with different software and ask for a particular performance reqriments. The general tendency of the service providers have been to be conservative and over reserve the resource to provide a particular performance which leads to resource wastage. Though there has been numerous attempts by the system software groups around the world to come up with dynamic resource scaling (for more detials look at \cite{naskos2016cloud}), the administrators would still like to start the system with the optimal configurations. This means before the system starts the service provider needs to run various bench mark runs to enable to choose the right configurations. Various runs of the benchmarks translates into wasted time and resources. The effort required for this task is exponentiated if the conifigurations have to be optimized for multiple performance measure such as energy, infrastructure cost etc. \cite{neha2016}. Since there is no concrete mathematical models available for such problem it make is ver difficult to solve. Since performing a exhaustive search of the possible options is not feasible, researchers over the years have come up with various strategies like stochastic search, evoluationary search etc. 
...
\item{Non-Separability: A problem is called non-seperable if the optimum of one of the objectives is not the optimun for the other obejctive/s.}
...
\item{Expensive: A problem is called expensive if the cost of evaluating a problem (or a function) is more costly with respect to any measures eg. time or money with respect to the cost of optimization.}
 ...
 In this report we would be biased towards evolutionary algorithm because if the general nature of the algorithms and effectiveness in many real world problems. \textcolor{red}{what is the difference between stochastic search and EA? Sampling is used to choose the members for the next iteration/generation where as EA operations (like NDSort, Niching) }
...
dense Pareto front, one must mandatorily preserve the population diversity, and avoid
discarding too easily the solutions which are dominated w.r.t. the current objectives.
Despite their shortcomings, some solutions are found to pave the way toward truly nondominated
...
We have seem how EAs can be very useful to solve problems with relative ease and absolutely minimum knowledge about the domain. But EAs have been plagued with the problems of slow convergence and requiring a lot of function evaluations. When function evaluations are \'expensive\' as explained earlier it is infeasible to run thousands of evaluations  to get the right results. There are various ways used to solve this problem:
\begin{itemize}
\item{Active learning based techniques like ~\cite{krall2014gale}. GALE, an optimizer that identifies and evaluates just
...
post-processor with its tool called WHERE, which explores only two evaluations per recursive split of the
...
design can take hours or even days.”. Harman \cite{harman2012search} comments
...
The first step involved initial points that are sampled, and then evaluatied with the computationally expensive functions in step2. After this, function values of the sampling points are available. In Step 3, the initial surrogate problem is constructed. In order to capture the Pareto frontier or to provide the most preferred solution based on the preference of a user (DM-Decision Maker), new samples are generated in Step 4. These points can be obtained by solveing the surrogate problem and/or sampling unexplored regions in the decision and/or objective space depending on a sampling process. In accordance with a method-dependent criterion, a subset of points among the generated points is selected. In Step 5, accessing a stop criterion may required to evaluated the selected sample points with computationally expensive functions and/or to update the surrogate problem. If a stopping criterion is met, the set of non-dominated solutions or the most preferred solution of the last surrogate problem which maybe have been evaluated with the computationally  expensive functions are considered as the approximated Pareto Frontier of the original problem or the most preferred soliton for a DM in Step 6. When the stopping criterion is not met, if the surrogate problem has already been updated, Step 4-5 are repeated. If not, first the surrogate problem is updated with the evaluated sample points and then Step4-5 are repeated.
 ...
 \subsection{Contriubtions of this thesis}
...
\textcolor{red}{put it stuff from \cite{tabatabaei2015survey}}
